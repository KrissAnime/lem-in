...
/*   Updated: 2018/08/21 11:33:43 by cbester          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	march(t_ant **ant, size_t x)
{
	char	*temp;
...
static void	marching(t_ant **ant, size_t x)
{
	size_t	i;
	...
	while ((*ant)->path[i])
	{
		if ((*ant)->path[i][ft_strlen((*ant)->path[i]) - 1])
	}
}
...
void		final_task(t_ant **ant)
{
	...
	add_space(ant);
	while (ft_atoi(&(*ant)->path[i][ft_strlen((*ant)->path[i]) - 1])
			!= (*ant)->ants)
	{
		march(ant, i);
		marching(ant, x);
